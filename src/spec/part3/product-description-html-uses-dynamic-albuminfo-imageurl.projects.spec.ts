...
describe('ProductDescription', () => {
  ...
  it(`should use image url data from the giftProductInfo property in the HTML template @product-description-html-uses-dynamic-albuminfo-imageurl`, async(() => {
    ...
    if (htmlString != "") {
      ...
      const re = /{{\s*giftProductInfo\?\.album\.coverImage\s*}}/
      since('We\'d like you to query the giftProductInfo property directly for the cover image, and we\'re not seeing that you\'re doing that.').expect(htmlDoc.querySelector('img').getAttribute('src').match(re)).toEqual(jasmine.any(Array));
    } else {
      since('We\'d like you to query the giftProductInfo property directly for the cover image, and we\'re not seeing that you\'re doing that.').expect(0).toBe(1);
    }
